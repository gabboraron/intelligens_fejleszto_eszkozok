...
for i=1:l
    ...
    if Adaptiv==1 && i>3
        qxDef_pp[i] = mimo(qxDef_pp[i], qx_p[i], qxDes_pp[i], δt, K, B, A₁)
        qyDef_pp[i] = mimo(qyDef_pp[i], qy_p[i], qyDes_pp[i], δt, K, B, A₂)
        qzDef_pp[i] = mimo(qzDef_pp[i], qz_p[i], qzDes_pp[i], δt, K, B, A₃)
...
figure(4)
grid(true)
plot(qN_p[1:l],qxN_p[1:l])
plot(qN_p[1:l],qyN_p[1:l])
plot(qN_p[1:l],qzN_p[1:l])
plot(qN_p[1:l],qx_p[1:l])
plot(qN_p[1:l],qy_p[1:l])
plot(qN_p[1:l],qz_p[1:l])
#plot(q_p[1:l],q[1:l],"r--")
display(gcf())
